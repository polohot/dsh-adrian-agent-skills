 ...
 * `ags-persona-*` skills, and 9 `ags-*` slash commands. Every skill keeps
 * `modelInvocable: true` and `userInvocable: false`, so the model discovers and
 * loads them while the human-facing command list shows only the 9 entry points.
 *
 * A command handler injects a DELEGATION BRIEF, never a skill body. A skill body
 * is 12 KB to 21 KB, and the main agent does not need it: the child loads the
 * skill itself. The main agent only packs context and merges the results. The
 * host-free half of that logic lives in `lib/brief.js`. See
...
import { PERSONA_SKILL_PREFIX, childSkillNames, planChildren, renderBrief } from './brief.js';
...
export function apply(ctx, config = {}) {
  ...
  ctx.logger?.info?.(`${name}: registered ${skills.length} skills, ${personaSkills.length} personas and ${commands.length} commands`);
}
