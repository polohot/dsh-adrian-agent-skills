 ...
 * them and the human slash list stays clean. The 9 entry skills are the mirror
 * image: `modelInvocable: false` and `userInvocable: true`, so they are the
 ...
 *
 * Each one is user-invocable and NOT model-invocable. That split matters twice.
 * The human slash list shows exactly these 9 names, and the model catalog stays
 * free of them, so a child can never trigger a second round of delegation.
 * @param {import('@deepseek-ai/cordis').Context} ctx - host context.
